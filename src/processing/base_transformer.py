...
class BaseTransformer(ABC):
    ...
    def _get_silver_metadata(self, metadata_key: str) -> Optional[Dict]:
        ...
        Metadata tracks:
        - source_files: dict of {bronze_key: source_digest}
    ...
    def _get_object_digest(self, s3_key: str) -> Optional[str]:
        """
        Get a stable digest for an S3 object.
        
        Prefers the custom ``content-sha256`` metadata set during Bronze
        ingestion.  Falls back to the S3 ETag (always present) so that
        Silver → Gold caching also detects source changes.
        
        :param s3_key: S3 key for the file
        :return: digest string or None if the object does not exist
        """
        try:
            response = self.s3.head_object(Bucket=self.bucket, Key=s3_key)
            metadata = response.get("Metadata", {})
            digest = metadata.get("content-sha256")
            if digest:
                return digest
    ...
    def _check_sources_changed(self, metadata_key: str, source_keys: List[str]) -> tuple[bool, List[str]]:
        ...
        for s3_key in source_keys:
            current_hash = self._get_object_digest(s3_key)
        ...
        for s3_key in source_keys:
            if s3_key not in tracked_sources and self._get_object_digest(s3_key):
    ...
    def _get_state_abbrev(self, state_code: str) -> str:
        ...
        return state_abbrev_map.get(state_code, 'XX')
