...
class TestIBGEIngestor(unittest.TestCase):
        ...
        self.config_data = {
            ...
            "datasets": [
                ...
                },
                {
                    ...
                    "bronze_prefix": "bronze/economic",
                    "filename": "test_ipca.json"
                }
            ]
        }
    ...
    def test_load_config(self, mock_boto3):
        ...
        self.assertEqual(len(ingestor.config["datasets"]), 3)
    ...
    def test_file_is_valid_exists_and_matches(self, mock_boto3):
        ...
        # Mock S3 head_object to return matching content digest metadata
        self.mock_s3.head_object.return_value = {
            'Metadata': {'content-sha256': 'abc123'}
        }
    ...
    def test_file_is_valid_exists_but_different(self, mock_boto3):
        ...
        # Mock S3 head_object to return different content digest metadata
        self.mock_s3.head_object.return_value = {
            'Metadata': {'content-sha256': 'different123'}
        }
    ...
    @patch('src.ingestion.ibge_client.boto3.client')
    def test_build_dataset_url_uses_full_url_when_present(self, mock_boto3):
        """Test that datasets can bypass SIDRA URL composition."""
        mock_boto3.return_value = self.mock_s3

        ingestor = IBGEIngestor("test-bucket", self.config_path)
        dataset = self.config_data["datasets"][2]

        assert ingestor._build_dataset_url(self.config_data["api_base_url"], dataset) == dataset["url"]

    @patch('src.ingestion.ibge_client.boto3.client')
    def test_build_s3_key_respects_custom_prefix(self, mock_boto3):
        """Test that datasets can land outside bronze/ibge when configured."""
        mock_boto3.return_value = self.mock_s3

        ingestor = IBGEIngestor("test-bucket", self.config_path)
        dataset = self.config_data["datasets"][2]

        assert ingestor._build_s3_key(dataset) == "bronze/economic/test_ipca.json"
    
    @patch('src.ingestion.ibge_client.boto3.client')
    @patch('src.ingestion.http_client.requests.get')
    def test_run_full_ingestion_skips_existing(self, mock_get, mock_boto3):
        """Test that ingestion skips files that already exist with matching content digest."""
        mock_boto3.return_value = self.mock_s3
        ...
        # Calculate expected SHA-256 metadata value
        import hashlib
        expected_digest = hashlib.sha256(test_content.encode('utf-8')).hexdigest()
        
        # Mock S3 to return matching object metadata
        self.mock_s3.head_object.return_value = {
