...
class TestSmartCaching:
    ...
    def test_get_object_digest_exists(self, transformer):
        """Test getting SHA-256 hash from existing bronze file."""
        transformer.s3.head_object.return_value = {
            'Metadata': {'content-sha256': 'abc123def456'}
        }
        
        result = transformer._get_object_digest('bronze/test/file.json')
    ...
    def test_get_object_digest_etag_fallback(self, transformer):
        """Test ETag fallback when content-sha256 metadata is absent (Silver files)."""
        transformer.s3.head_object.return_value = {
            'ETag': '"etag_abc123"',
            'Metadata': {}
        }

        result = transformer._get_object_digest('silver/fact_income/data.parquet')

        assert result == "etag_abc123"

    def test_get_object_digest_prefers_sha256(self, transformer):
        """Test that content-sha256 is preferred over ETag when both are present."""
        transformer.s3.head_object.return_value = {
            'ETag': '"etag_ignored"',
            'Metadata': {'content-sha256': 'sha256_wins'}
        }

        result = transformer._get_object_digest('bronze/test/file.json')

        assert result == "sha256_wins"

    def test_get_object_digest_not_found(self, transformer):
        """Test getting hash for non-existent file."""
        transformer.s3.head_object.side_effect = ClientError(
            {'Error': {'Code': '404'}}, 'HeadObject'
        )
        
        result = transformer._get_object_digest('bronze/test/missing.json')
    ...
    def test_check_sources_changed_no_changes(self, transformer):
        ...
        # Mock head_object to return same hashes via custom metadata
        def head_object_side_effect(Bucket, Key):
            if Key == 'bronze/test/file1.json':
                return {'Metadata': {'content-sha256': 'hash1'}}
            elif Key == 'bronze/test/file2.json':
                return {'Metadata': {'content-sha256': 'hash2'}}
    ...
    def test_check_sources_changed_file_modified(self, transformer):
        ...
        def head_object_side_effect(Bucket, Key):
            if Key == 'bronze/test/file1.json':
                return {'Metadata': {'content-sha256': 'hash1_new'}}
            elif Key == 'bronze/test/file2.json':
                return {'Metadata': {'content-sha256': 'hash2'}}
    ...
    def test_check_sources_changed_new_file(self, transformer):
        ...
        def head_object_side_effect(Bucket, Key):
            if Key == 'bronze/test/file1.json':
                ...
                return {'Metadata': {'content-sha256': 'hash2_new'}}
...
class TestTransparencySmartCaching:
    ...
    def test_should_skip_processing_no_changes(self, transformer):
        ...
        # Source file hash matches via custom metadata
        def head_object_side_effect(Bucket, Key):
