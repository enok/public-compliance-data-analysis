...
class TestIBGETransformer:
        ...
        config = {
            ...
            "schemas": {
                ...
                "inflation_index": {
                    "columns": {
                        "year": {"type": "integer"},
                        "annual_ipca_rate_pct": {"type": "float"},
                        "ipca_index_avg": {"type": "float"},
                        "ipca_index_dec": {"type": "float"},
                        "deflator_to_2022": {"type": "float"},
                        "reference_base_year": {"type": "integer"},
                        "last_reference_date": {"type": "date"}
                    }
                }
            },
            "state_mapping": {
    ...
    def test_get_source_datasets(self, mock_schema_config):
        ...
        assert 'inflation' in datasets

    def test_build_annual_inflation_reference(self, mock_schema_config):
        """Test annual inflation reference generation and 2022 deflation factors."""
        from src.processing.ibge_transformer import IBGETransformer
        ...
        inflation_data = [
            {"data": "01/01/2010", "valor": "0.50"},
            {"data": "01/02/2010", "valor": "0.25"},
            {"data": "01/01/2022", "valor": "0.75"},
            {"data": "01/02/2022", "valor": "1.00"}
        ]

        result = transformer._build_annual_inflation_reference(inflation_data)

        assert list(result['year']) == [2010, 2022]
        assert result.loc[result['year'] == 2022, 'deflator_to_2022'].iloc[0] == 1.0
        assert result.loc[result['year'] == 2010, 'deflator_to_2022'].iloc[0] > 1.0

    def test_build_annual_inflation_reference_empty(self, mock_schema_config):
        """Test that empty or None inflation data returns an empty DataFrame."""
        from src.processing.ibge_transformer import IBGETransformer

        with patch('boto3.client'):
            transformer = IBGETransformer("test-bucket", mock_schema_config)

        assert transformer._build_annual_inflation_reference(None).empty
        assert transformer._build_annual_inflation_reference([]).empty

    def test_build_annual_inflation_reference_missing_base_year(self, mock_schema_config):
        """Test that a missing base year returns an empty DataFrame."""
        from src.processing.ibge_transformer import IBGETransformer
        ...
        result = transformer._build_annual_inflation_reference(inflation_data, base_year=2022)
        assert result.empty
